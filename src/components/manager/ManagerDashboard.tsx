 ...
 * Main dashboard for managers to view team emotion analytics
 */

import React, { useEffect, useState } from 'react';
import { useAuth } from '../../contexts/AuthContext';
import { getTeamEmotionLogs } from '../../services/firestore.service';
...
export function ManagerDashboard(): JSX.Element {
  ...
  const [filteredLogs, setFilteredLogs] = useState<EmotionLog[]>([]);
  ...
  useEffect(() => {
    async function fetchTeamLogs() {
      ...
      try {
        setLoading(true);
        const teamLogs = await getTeamEmotionLogs(
          currentUser.teamId,
          startDate,
          endDate
        );
        setLogs(teamLogs);
        setFilteredLogs(teamLogs);
        setError(null);
      } catch (err: any) {
        setError(err.message || 'Failed to fetch team emotion logs');
      } finally {
        setLoading(false);
      }
    }
  ...
  return (
    <div className="manager-dashboard">
      ...
      <section className="analytics-section">
        ...
        <TeamAnalytics logs={filteredLogs} />
      </section>
      ...
      <div className="dashboard-info">
        ...
        <div className="info-card tips">
          ...
          <ul>
            <li>
              <strong>High negative emotions:</strong> May indicate stress or burnout - consider
              team check-ins or workload adjustments
            </li>
            <li>
              <strong>Low detection count:</strong> Team members may not be using the system
              regularly
            </li>
            <li>
              <strong>Trends over time:</strong> Look for patterns in specific time periods or days
            </li>
            <li>
              <strong>Balance is key:</strong> A mix of emotions is normal - focus on sustained
              negative trends
            </li>
          </ul>
        </div>
