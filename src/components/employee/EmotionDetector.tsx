 ...
 * Displays webcam feed and real-time emotion detection
 */

import React, { useEffect } from 'react';
import { useEmotionDetection } from '../../hooks/useEmotionDetection';
import { useAuth } from '../../contexts/AuthContext';
import { saveEmotionLog } from '../../services/emotion.service';
import { EMOTION_COLORS } from '../../utils/constants';

export function EmotionDetector(): JSX.Element {
  const { prediction, isLoading, error, videoRef, startDetection, stopDetection, isActive } =
    useEmotionDetection();
  const { currentUser } = useAuth();

  // Auto-save emotion logs when prediction changes
  useEffect(() => {
              ...
              {prediction.faceDetected ? (
                <div className="prediction-info">
                  <div
                    ...
                    style={{
                      backgroundColor: EMOTION_COLORS[prediction.dominantEmotion],
                    }}
                  >
                              ...
                              backgroundColor:
                                EMOTION_COLORS[
                                  (emotion.charAt(0).toUpperCase() +
                                    emotion.slice(1)) as keyof typeof EMOTION_COLORS
                                ],
                            }}
                          ></div>
                        </div>
