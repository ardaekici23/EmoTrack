...
export function EventDetector({ eventId }: EventDetectorProps) {
  const { prediction, isLoading, error, videoRef, startDetection, stopDetection, isActive } =
    useEmotionDetection();
  const { currentUser } = useAuth();

  // Auto-start when models are ready
  useEffect(() => {
    if (!isLoading) startDetection();
    return () => stopDetection();
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [isLoading]);

  // Save each prediction tagged with the event
  useEffect(() => {
    if (prediction && currentUser && prediction.faceDetected) {
      saveEmotionLog(currentUser.userId, prediction, eventId);
    }
  }, [prediction, currentUser, eventId]);
    ...
    <div className="emotion-detector">
              ...
              {prediction.faceDetected ? (
                <div className="prediction-info">
                  <div
                    ...
                    style={{ backgroundColor: EMOTION_COLORS[prediction.dominantEmotion] }}
                  >
      ...
      {!isActive && (
        <div className="detection-placeholder">
          <p>Click "Start Detection" to resume emotion tracking for this event.</p>
        </div>
      )}
    </div>
  );
